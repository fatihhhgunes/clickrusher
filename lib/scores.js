...
// UTC bazlı gün string'i: "2026-06-14"
...
async function processGames(games) {
  ...
  const yesterdayUTC = utcDay(new Date(now - 24 * 3600 * 1000));

  // UTC kickoff zamanına göre filtrele: dün veya bugün UTC
  // Neden dün de dahil: Batı Kıyısı stadyumlarında yerel saat dün olabilir
  // (örn. 04:00 UTC → 21:00 PT önceki gün) ama İstanbul'da bugündür
  const relevantGames = games.filter(g => {
    const utc = gameToUtc(g);
    if (!utc) return false;
    const day = utcDay(new Date(utc));
    return day === todayUTC || day === yesterdayUTC;
  });
  ...
  if (updated > 0) console.log(`[scores] ${updated} maç (${yesterdayUTC}/${todayUTC} UTC), ${autoFixtures.length} fikstür güncellendi`);
}
