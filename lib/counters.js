...
async function addCountryClicks(countryCode, deviceId, username, n) {
  ...
  await redis.hincrby(`user:countries:${deviceId}`, countryCode, n);
}

// Kullanıcının ülke bazlı tık istatistikleri: {TUR: 42, BRA: 7, ...}
async function getUserCountryStats(deviceId) {
  const raw = await redis.hgetall(`user:countries:${deviceId}`);
  if (!raw) return {};
...
async function updateUser(deviceId, username, totalClicks) {
  await redis.hset(`user:${deviceId}`, { name: username, total: String(totalClicks) });
}
